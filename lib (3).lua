...
local redzlib = {
	Themes = {
		...
		Blue = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8, 13, 20)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(40, 51, 69)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 13, 20))
	    	}),
	...
	Flags = {},
	Tabs = {},
	Icons = loadstring(game:HttpGet("https://raw.githubusercontent.com/haru7932/Lxnny-Hub/refs/heads/main/icons.lua",true))()
}
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddToggle(Configs)
			...
			local Toggle = InsertTheme(Create("Frame", Slider, {
				...
				Position = UDim2.new(0, 0, 0.5),
				AnchorPoint = Vector2.new(0, 0.5),
				BackgroundColor3 = Theme["Color Ball Theme"]
...
Text = ""
			...
			local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected , Selectd do
...
AddRemoveOptions = function(List, Clear)
if Clear then
table.foreach(List, RemoveOption)
else
...
function Dropdown:Selc(Sla)
Selectd(Sla)
end
			function Dropdown:Set(Val1, Clear)
				if type(Val1) == "table" then
					AddRemoveOptions(Val1, Clear)
				elseif type(Val1) == "function" then
					Callback = Val1
				end
			end
			function Dropdown:SetK(Val1, Clear)
				if type(Val1) == "table" then
    ...
    return Label
        ...
        function Tab:AddThemeDropdown()
            return Tab:AddDropdown({
                Name = "Theme",
                Description = "",
                Options = {"Blue", "Darker", "Dark", "Purple"},
                Default = "Blue",
                Callback = function(Value)
